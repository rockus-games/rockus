...
for (var i = 0; i < recommended_game_count; i++) {
  ...
  poster.innerHTML = `<img src="./assets/images/recommended/game${i+1}/image1.jpeg">`;
  ...
  poster_images.innerHTML = `<h2>Watch Dogs 2</h2>`;
  poster_images.innerHTML += `<div id="pimage${i}" class="pimage"></div>`;
  let pimage = document.querySelector(`#pimage${i}`);
  for (var j = 0; j < 4; j++){
    pimage.innerHTML += `<div onmouseover="poster_image_change(${j+2})" onmouseout="poster_image_unchange()" id="cartochka${j+1}" class="cartochki"><img width="144" height="125" src="./assets/images/recommended/game${i+1}/image${j+2}.jpeg"></div>`;
  }
}
...
function poster_image_change(x){
  ...
  poster.innerHTML = `<img src="./assets/images/recommended/game${current_recommended+1}/image${x}.jpeg">`;
}

function poster_image_unchange(){
  let poster = document.querySelector(`#poster${current_recommended+1}`);
  poster.innerHTML = `<img src="./assets/images/recommended/game${current_recommended+1}/image1.jpeg">`;
}